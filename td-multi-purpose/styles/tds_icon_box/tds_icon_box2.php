...
class tds_icon_box2 extends tds_api {
    ...
    private function get_css() {
	    ...
	    $icon_right = $this->get_style_att( 'icon_right' );

        if ( empty ( $icon_right ) ) {
            $td_css_compiler->load_setting_raw( 'icon_right_space', $this->get_style_att( 'icon_right_space' ) . 'px' );
        } else {
            $td_css_compiler->load_setting_raw( 'icon_left_space', $this->get_style_att( 'icon_right_space' ) . 'px' );
        }
    ...
    function render( $index_style = '' ) {
        ...
        $buffy .= '<div class="' . tds_api::get_group_style( __CLASS__ ) . ' ' . $this->get_class_style(__CLASS__) . ' ' . 'td-fix-index' . ' ' . $icon_position . ' ' . $this->unique_style_class . '">';
